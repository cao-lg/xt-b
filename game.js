...
const Game = (function () {
  ...
  function affinityMult(atkAff, defAff) {
    // 阳克阴(+15%), 阴克调(+15%), 调克阳(+15%)
    if (atkAff === '阳' && defAff === '阴') return 1.15;
    if (atkAff === '阴' && defAff === '调') return 1.15;
    if (atkAff === '调' && defAff === '阳') return 1.15;
    return 1;
  }
  ...
  function load() {
    ...
    try {
      ...
      if (raw) {
        ...
        console.log('[LOAD] data.ac='+data.autoCultivate+' state.ac='+state.autoCultivate);
        // 强制覆盖关键开关
        if (typeof data.autoCultivate === 'boolean') state.autoCultivate = data.autoCultivate;
        console.log('[LOAD] after fix state.ac='+state.autoCultivate);
